...
function apply(ctx) {
  ...
  `;
  const expandedTurns = /* @__PURE__ */ new Set();
  ...
  function applyBox(box, seen, withOfficial, toolCalls, withRow) {
    ...
    visible.forEach((el, k) => {
      ...
      if (k > 0) {
        ...
        const gapOn = sameFlow ? el : el.closest(FLOW);
    ...
    withRow.add(key);
    const beforeEl = box[0].closest(FLOW);
  ...
  function applyFold() {
    ...
    try {
      ...
      for (const box of boxes) {
        const key = keyOf(box[0].closest(FLOW));
  ...
  function onDocumentClick(event) {
    const row = event.target?.closest?.(OFFICIAL);
    if (!row) return;
