...
折叠时藏了内容的框，会在上边框之上再堆叠两条线，提示"里面还有东西"。这两条线本身就是控件：点它只展开**这一个框**（就地长出来，不做滚动跳转）。每个框各自记住自己的展开状态；每轮那行开关（或官方按钮）仍然一次开关这一轮的全部框。
