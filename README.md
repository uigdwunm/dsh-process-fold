...
When a collapsed box is hiding items, two lines are stacked above its top border, so you can see it holds more than it shows. Click those lines to expand that one box in place. Once expanded, the same top border becomes the collapse control: hover the box's top edge and a merge/collapse icon appears in the middle of the line — click the line to fold that one box back. Each box keeps its own expanded state; the per-turn row (or the official button) still opens and closes all of that turn's boxes at once.
...
- Turns that same top border into the collapse control once the box is open: hovering the top edge fades in a merge icon on the line, and clicking the line folds that one box back (the collapsed items disappear in place, so nothing jumps).
...
折叠时藏了内容的框，会在上边框之上再堆叠两条线，提示"里面还有东西"。点这两条线只展开**这一个框**（就地长出来，不做滚动跳转）。展开之后，同一条上边框反过来就是收回控件：鼠标移到框的上边缘，线中间会浮出一个"合并/收起"图标，点这条线就把这一个框收回去（同样不跳）。每个框各自记住自己的展开状态；每轮那行开关（或官方按钮）仍然一次开关这一轮的全部框。
...
- 展开后同一条上边框变成收回控件：鼠标移到框的上边缘时线上的合并图标淡入，点这条线只把这一个框收回去（藏起来的项原地消失，不会跳）。
